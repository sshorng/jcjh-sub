...
(async function run() {
  ...
  try {
    ...
    assert.match(root.body.toString('utf8'), /domain-match\.js\?v=20260831-combined1/);
    assert.match(root.body.toString('utf8'), /ui-request\.js\?v=20260831-combined1/);
    assert.match(root.body.toString('utf8'), /20260831-combined1/);
    assert.equal(root.headers['cache-control'], 'no-cache');
    assert.match(root.headers['content-security-policy'], /frame-ancestors 'none'/);

    const app = await request(server, 'GET', '/app.js?v=20260831-combined1');
    assert.equal(app.status, 200);
    assert.match(app.body.toString('utf8'), /params\.set\('response_type', 'id_token token'\)/);

    const head = await request(server, 'HEAD', '/style.css?v=20260831-combined1');
