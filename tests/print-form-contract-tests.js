...
vm.runInContext(fs.readFileSync(path.join(root, 'print-helper.js'), 'utf8'), context, {
...
assert.equal((packed.match(/copy me/g) || []).length, 2, 'each original form must print as two identical copies');
assert.equal((packed.match(/class="print-page"/g) || []).length, 1);
...
assert.match(context.window.getPrintPreviewCss(), /official-serial-mark \{[^}]*left: 0;[^}]*text-align: left/);
